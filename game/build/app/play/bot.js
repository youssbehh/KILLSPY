...
export default function GameScreen() {
    ...
    const getRandomAction = () => {
        const actions = [motTraduit(langIndex, 42), motTraduit(langIndex, 41), motTraduit(langIndex, 40)];
        if (playerMunitions === 0)
            return motTraduit(langIndex, 42);
        if (playerMunitions === 3)
            return actions[1 + Math.floor(Math.random() * 2)];
        return actions[Math.floor(Math.random() * 3)];
    };
    // Décompte de 5 secondes pour l'action
    useEffect(() => {
        if (gameOver)
            return; // Arrêter si game over
    ...
    const executeBotAction = () => {
        ...
        if (botMunitions === 0) {
            ...
            if (random < 0.33)
                action = motTraduit(langIndex, 42); // 33% recharger
            else if (random < 0.66)
                action = motTraduit(langIndex, 41); // 33% tirer
            else
                action = motTraduit(langIndex, 40); // 33% bouclier
        }
        setBotAction(action);
    };
    ...
    return (<>
        ...
        <View style={styles.container}>
            {gameOver ? (<View style={styles.gameOverContainer}>
                    <Text style={styles.gameOverText}>
                        {isWinner === null ? motTraduit(langIndex, 32) : // Match nul
                isWinner ? motTraduit(langIndex, 36) : motTraduit(langIndex, 37)}
                    </Text>
                    <View style={styles.buttonsRow}>
                        <Pressable style={styles.restartButton} onPress={() => router.push('/play/bot')}>
                            <Text style={styles.buttonText}>{motTraduit(langIndex, 38)}</Text>
                        </Pressable>
                        <Pressable style={styles.restartButton} onPress={() => router.replace('/(tabs)/gamechoice')}>
                            <Text style={styles.buttonText}>{motTraduit(langIndex, 43)}</Text>
                        </Pressable>
                    </View>
                </View>) : !isGameStarted ? (<Text style={styles.countdown}>{countdown}</Text>) : (<>
                    ...
                    <View style={styles.buttonsContainer}>
                        <Pressable style={[styles.button, playerAction === motTraduit(langIndex, 42) && styles.selectedButton]} onPress={() => setPlayerAction(motTraduit(langIndex, 42))} disabled={actionCountdown === 0 || playerMunitions === 3}>
                            <Text style={styles.buttonText}>{motTraduit(langIndex, 42)}</Text>
                        </Pressable>
                        <Pressable style={[styles.button, playerAction === motTraduit(langIndex, 41) && styles.selectedButton]} onPress={() => setPlayerAction(motTraduit(langIndex, 41))} disabled={actionCountdown === 0 || playerMunitions === 0}>
                            <Text style={styles.buttonText}>{motTraduit(langIndex, 41)}</Text>
                        </Pressable>
                        <Pressable style={[styles.button, playerAction === motTraduit(langIndex, 40) && styles.selectedButton]} onPress={() => setPlayerAction(motTraduit(langIndex, 40))} disabled={actionCountdown === 0}>
